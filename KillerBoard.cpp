...
bool KillerBoard::solve(){
    ...
    for (int i=1; i<=NUM; ++i){
        if (canPlace(col, row, i)){
            data_[col][row] = i;
            int sectorIndex = sectors[std::pair<int,int>(col,row)];
            sectorSizes[sectorIndex].first += i;
...
bool KillerBoard::canPlace(int col, int row, int num){
    ...
    && (sectorSizes[sectorIndex].first + num <= sectorSizes[sectorIndex].second));
}
...
void KillerBoard::loadBoard(std::ifstream &file){
    ...
    while (file >> y){
        ...
        if (y < 0){
            sectorIndex = (-1*y)-1;
            sectorMax = x;
            sectorSizes.push_back(std::pair<int,int>(0, sectorMax));
            continue;
        }
        sectors[std::pair<int,int>(y,x)] = sectorIndex;
    }
}